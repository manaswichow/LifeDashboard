...
// REPLACE from Line 178 to Line 198 (inclusive)
...
function renderTasks(filter = 'all') {
    ...
    DB.tasks.filter(t => {
        ...
        // --- MODIFICATION: Apply strikethrough class if the task is done ---
        if (t.done) {
...
// REPLACE from Line 335 to Line 355 (inclusive)

function renderGoals() {
    goalsList.innerHTML = '';
    DB.goals.forEach(g => {
        // --- MODIFICATION: Check if progress is completed ---
        const isCompleted = g.target > 0 && g.progress >= g.target;

        const li = document.createElement('div'); li.className = 'list-item';

        // --- MODIFICATION: Apply strikethrough class if the goal is completed ---
        if (isCompleted) {
            li.classList.add('completed-item');
        }
